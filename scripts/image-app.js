(function(){
  ...
  function manipulateImage(type) {
    var imageManipulationWorker = new Worker('../scripts/worker.js') ; 
    ...
    imageManipulationWorker.postMessage({imageData: imageData, type: type});
    // receive messages from the web worker.
    imageManipulationWorker.onmessage = function(e){
      toggleButtonsAbledness();      
      img = e.data;
      if(img) return ctx.putImageData(img, 0, 0);
    };
    imageManipulationWorker.onerror = function(e){
      throw 'image manpulation error';
    };
  };
